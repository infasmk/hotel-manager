...
const Income: React.FC = () => {
    ...
    const formInputClass = "block w-full px-6 py-4 rounded-2xl border border-white/10 bg-black/60 text-sm font-bold text-white transition-all focus:outline-none focus:ring-4 focus:ring-primary-500/10 focus:border-primary-500 placeholder:text-gray-600 appearance-none";
    ...
    return (
        <div className="space-y-6 sm:space-y-10 animate-in fade-in slide-in-from-bottom-2 duration-700">
            ...
            <div className="bg-emerald-500/10 border border-emerald-500/20 p-6 sm:p-10 rounded-[2.5rem] flex items-center justify-between shadow-2xl">
                <div className="flex items-center gap-5">
                    ...
                    <div>
                        ...
                        <p className="text-[11px] font-bold text-gray-400 uppercase tracking-widest">{filteredIncome.length} Records</p>
                    </div>
                </div>
            ...
            <div className="bg-gray-950/90 p-4 sm:p-6 rounded-[2.5rem] border border-white/5 shadow-2xl backdrop-blur-2xl sticky top-[6.5rem] z-[80]">
                <div className="flex flex-col gap-4">
                    <div className="relative group w-full">
                        ...
                        <input
                            type="text"
                            placeholder="Search daily ledger..."
                            value={searchTerm}
                            onChange={(e) => setSearchTerm(e.target.value)}
                            className="w-full pl-14 pr-6 py-4 text-sm bg-black/40 border border-white/5 rounded-2xl focus:outline-none focus:ring-4 focus:ring-primary-500/10 transition-all text-white font-bold"
                        />
                    </div>
                    <div className="flex gap-4">
                        <button
                            onClick={() => setFilterByDate(!filterByDate)}
                            className={`flex-1 flex items-center justify-center gap-2 px-6 py-4 text-xs font-black rounded-2xl transition-all border uppercase tracking-[0.2em] ${filterByDate ? 'bg-primary-500 text-white border-primary-500 shadow-lg shadow-primary-500/20' : 'bg-white/5 border-white/10 text-gray-500 hover:text-white'}`}
                        >
                            <Filter size={16} />
                            {filterByDate ? 'Today' : 'All'}
                        </button>
                        <button
                            onClick={() => handleOpenModal()}
                            className="flex-1 flex items-center justify-center gap-3 px-6 py-4 text-white bg-primary-600 rounded-2xl hover:bg-primary-700 transition-all font-black text-xs uppercase tracking-[0.3em] shadow-xl shadow-primary-600/20 active:scale-95 group"
                        >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {isLoading ? (
                            ...
                            >
                                ...
                                {item.notes && (
                                    <div className="mb-6">
                                        ...
                                        >
                                            {isExpanded ? <><ChevronUp size={10}/> Less</> : <><ChevronDown size={10}/> View More</>}
                                        </button>
                                    </div>
                                )}
                    ...
                    <div className="col-span-full py-24 text-center">
                        <p className="text-[11px] font-black text-gray-700 uppercase tracking-[0.5em]">No Credit Records Found</p>
                    </div>
                )}
            </div>

            <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={currentIncome?.id ? 'Revision' : 'Log Income'}>
                <form onSubmit={handleSave} className="space-y-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-[10px] font-black text-gray-500 uppercase tracking-widest ml-1">Narrative Details</label>
                        <textarea 
                            placeholder="Add specific details (e.g., Room 102, Dinner service)..." 
                            value={currentIncome?.notes || ''} 
                            onChange={(e) => setCurrentIncome({ ...currentIncome, notes: e.target.value })} 
                            className={`${formInputClass} min-h-[160px] resize-none pt-4 leading-relaxed`}
                        ></textarea>
                    </div>
                    <div className="flex gap-4 pt-10 border-t border-white/5">
                        <button type="button" onClick={handleCloseModal} className="flex-1 py-5 text-[11px] font-black text-gray-500 bg-white/5 rounded-2xl hover:text-white transition-all uppercase tracking-widest border border-white/5">Cancel</button>
                        <button type="submit" className="flex-1 py-5 text-[11px] font-black text-white bg-primary-600 rounded-2xl shadow-xl shadow-primary-600/20 hover:bg-primary-700 uppercase tracking-widest transition-all">Authorize</button>
                    </div>
                </form>
            </Modal>
            ...
            <ConfirmModal 
                ...
                title="Wipe Entry?"
                message="This entry will be permanently removed from the centralized audit trail."
            />
        </div>
    );
};
